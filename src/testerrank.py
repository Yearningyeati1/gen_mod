...
class Tester(object):
    ...
    def getrealoccsamples(self, allimages, numface, image_names, predmeshes):
        ...
        for i in range(len(image_names)):
            ...
            arcface = torch.tensor(np.load(re.sub('jpg','npy', images))).float().to('cpu')
            ...
            with torch.no_grad():
                if numface > 1:
                    arcface = arcface.unsqueeze(0).to('cpu')
                    img_tensor = torch.Tensor(normtransimage).unsqueeze(0).to('cpu')
                    imgfarl_tensor = torch.Tensor(imagefarl).unsqueeze(0).to('cpu')
                    codedict, _ = self.model.encode(img_tensor, arcface, imgfarl_tensor)
                else:
                    codedict, _ = self.model.encode(torch.Tensor(origimage).unsqueeze(0).to('cpu'), arcface.unsqueeze(0))
                for j in range(numface):
                    mesh = trimesh.load(predmesh+'_'+str(j)+'.ply')
                    allpredmesh.append(torch.tensor(np.array(mesh.vertices)).float())
                allpredmesh = torch.stack(allpredmesh).to('cpu')
    ...
    def getmaxsampleindex(self, arcface, normtransimage, imagefarl, allpredmesh, numface=10):#changed numface 100->10 on 29.10.2025
            arcface = arcface.unsqueeze(0).to('cpu')
            img_tensor = torch.Tensor(normtransimage).unsqueeze(0).to('cpu')
            imgfarl_tensor = torch.Tensor(imagefarl).unsqueeze(0).to('cpu')
            codedict, _ = self.model.encode(img_tensor, arcface, imgfarl_tensor)
            allpredmesh = 1000*(allpredmesh).to('cpu')
