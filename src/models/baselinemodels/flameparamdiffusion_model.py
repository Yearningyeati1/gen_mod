...
class FlameParamDiffusionModel(BaseModel):
    ...
    def create_model(self, model_config):
        ...
        if self.expencoder == 'arcface':
            ...
            self.dinov2model.load_state_dict(torch.load('pretrained/dinov2_vitl14.pth',map_location=torch.device('cpu')))
    ...
    def decode(self, codedict, epoch=0, visualize=False, withpose=False, withexp=False,shapecode=None, expcode=None, rotcode=None, numsamples=1):
        ...
        print("Available keys in codedict:", codedict.keys())
        print("Expected expencoder:", self.expencoder)

        if self.expencoder == 'arcfarl':
            identity_code = torch.cat((codedict['farl'], F.normalize(codedict['arcface'])), dim=1)
        else:
